...
class Profiler(trt.infer.Profiler):
    ...
    def print_layer_times(self):
        ...
        for i in range(len(self.profile)):
            print(
                "{:40.40} {:4.3f}ms".format(
                    self.profile[i][0], self.profile[i][1] / self.timing_iterations
                )
            )
            totalTime += self.profile[i][1]
        print(
            "Time over all layers: {:4.2f} ms per iteration".format(
                totalTime / self.timing_iterations
            )
        )
...
def time_inference(engine, batch_size, inp):
    ...
    for io in get_input_output_names(engine):
        inp_idx = create_memory(engine, io, bindings, mem, batch_size, inp, inp_idx)
