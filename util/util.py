...
def gds_to_img(filename, dirpath, poly_layers = [0, 1], true_design_size = 6.144):
      ...
      if(design_size[0] > permitted_size or design_size[1] > permitted_size):
          raise ValueError("Incorrect GDS size. Design size is %s"%(design_size), f"must be less than or equal to {permitted_size}um x {permitted_size}um")
      ...
      for each_polygon in zx.get_polygons():
          ...
          poly_pts = [tuple((np.array([round(each,3) for each in each_vertex])-np.array([zx.xmin, zx.ymin])+np.array([(dss-zx.xsize)/2, (dss-zx.ysize)/2]))*sm) for each_vertex in each_polygon.tolist()]
...
def write_gds(polygon_v, filepath="litho_example.gds", cellname="litho", output_layers=[0]):
  #clear gds cell library for conflicts
  [gdspy.current_library.remove(each_cell) for each_cell in list(gdspy.current_library.cells)]
  ...
  for idx, val in enumerate(output_layers):
    ...
    for i in range(len(polygon_v[idx])):
      ...
      if(len(polygon_v[idx][i])>2):
        litho_polyinst = gdspy.boolean(litho_polyinst, gdspy.Polygon(temp_pts, layer = val, datatype=0), operation = 'xor', layer = val, datatype=0)
